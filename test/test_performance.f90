program test_performance
  ...
  integer :: n, i, ierr
  real, allocatable :: A(:,:), A_orig(:,:), b(:), x(:), x_exact(:)
  real :: start_time, end_time, residual, residual_pivot
  real :: original_time, pivot_time

  print *, "=== 性能测试 (比较原始算法和选主元算法) ==="
  print *, "矩阵大小, 原始时间(秒), 原始残差, 选主元时间(秒), 选主元残差"

  do n = 50, max_size, step
    allocate(A(n,n), A_orig(n,n), b(n), x(n), x_exact(n))

    ! 生成对角占优矩阵（保证可解）
    call generate_diagonal_dominant(A_orig, n, 10.0)  ! 增加对角优势
    call random_number(x_exact)
    x_exact = x_exact * 10.0
    b = matmul(A_orig, x_exact)  ! 计算精确解对应的b
    ...
    call cpu_time(end_time)
    if (ierr == 0) then
      residual_pivot = matrix_residual(A_orig, x, b)
    else
      residual_pivot = huge(1.0)
    end if
    pivot_time = end_time - start_time

    print '(I5, ", ", F10.6, ", ", E12.4, ", ", F10.6, ", ", E12.4)', &
          n, original_time, residual, pivot_time, residual_pivot

    deallocate(A, A_orig, b, x, x_exact)
  end do
  ...
  do n = 5, 20, 5
    ...
    ! 生成希尔伯特矩阵（著名的病态矩阵）
    call generate_hilbert_matrix(A_orig, n)
    call random_number(x_exact)
    x_exact = x_exact * 5.0
    b = matmul(A_orig, x_exact)
